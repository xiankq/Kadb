...
Future<void> _getDeviceModel(AdbConnection connection) async {
  ...
  try {
    // 执行getprop命令获取设备型号
    final shellStream = await KadbDart.executeShell(connection, 'getprop', ['ro.product.model']);
...
Future<void> _getDeviceInfo(AdbConnection connection) async {
  ...
  for (final entry in properties.entries) {
    try {
      final shellStream = await KadbDart.executeShell(connection, 'getprop', [entry.key]);
