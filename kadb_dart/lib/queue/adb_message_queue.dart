...
class AdbMessageQueue {
  ...
  Future<AdbMessage> takeWithTimeout(int localId, int command, {required Duration timeout}) async {
    ...
    // 检查是否还在监听该本地ID，如果不在监听，说明流已关闭
    if (!_openStreams.contains(localId)) {
      print('🔍 ADB消息队列: 本地ID 0x${localId.toRadixString(16)} 不在监听列表中（takeWithTimeout），当前监听的ID: ${_openStreams.map((id) => '0x${id.toRadixString(16)}').join(', ')}');
      
      // 修复：对于某些特殊情况，尝试重新添加到监听列表而不是立即抛出异常
      if (localId >= 4) { // 对于ID >= 4的流，可能是临时创建的测试流
        print('🔍 尝试重新添加本地ID 0x${localId.toRadixString(16)} 到监听列表（takeWithTimeout）');
        _openStreams.add(localId);
        if (!_queues.containsKey(localId)) {
          _queues[localId] = {};
        }
