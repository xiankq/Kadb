...
Uint8List get _clientName =>
    PairingAuthCtxFactory.getBytes('adb pair client\u0000', 'UTF-8');
Uint8List get _serverName =>
    PairingAuthCtxFactory.getBytes('adb pair server\u0000', 'UTF-8');

// 以下值取自Android源代码，可能会发生变化
// https://android.googlesource.com/platform//packages/modules/adb/+/main/pairing_auth/aes_128_gcm.cpp
Uint8List get _info =>
    PairingAuthCtxFactory.getBytes('adb pairing_auth aes-128-gcm key', 'UTF-8');
...
class Spake2ContextImpl implements Spake2Context {
  ...
  void _initialize() {
    ...
    _m = _hexToBytes(
      'd048032c6ea0b6d697ddc2d86c7d82252b33de61e6b028dbbf6a6a2c6f9f8e1',
    );
    _n = _hexToBytes(
      'd048032c6ea0b6d697ddc2d86c7d82252b33de61e6b028dbbf6a6a2c6f9f8e2',
    );
  }
  ...
  Uint8List _reduceModQ(Uint8List input) {
    ...
    var value = BigInt.parse(
      input
          ...
          .fold<String>(
            '',
            (prev, byte) => prev + byte.toRadixString(16).padLeft(2, '0'),
          ),
      radix: 16,
    );
    value = value % q;
...
class PairingAuthCtxFactory {
  ...
  static PairingAuthCtx? createAlice(Uint8List password) {
    try {
      ...
      final spake25519 = Spake2ContextImpl(
        Spake2Role.alice,
        _clientName,
        _serverName,
      );
