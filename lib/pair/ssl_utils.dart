...
class SslUtils {
  ...
  /// 验证数字签名
  static bool verifySignature(RSAPublicKey publicKey, Uint8List data, Uint8List signature) {
  ...
  /// 计算TLS主密钥
  static Uint8List computeTlsMasterSecret(Uint8List preMasterSecret, Uint8List clientRandom, Uint8List serverRandom) {
  ...
  /// 计算TLS密钥块
  static Uint8List computeTlsKeyBlock(Uint8List masterSecret, Uint8List clientRandom, Uint8List serverRandom, int length) {
    final seed = Uint8List(clientRandom.length + serverRandom.length);
    seed.setRange(0, clientRandom.length, clientRandom);
    seed.setRange(clientRandom.length, seed.length, serverRandom);
  ...
  /// PRF函数（伪随机函数）
  static Uint8List _prf(Uint8List secret, String label, Uint8List seed, int length) {
